			...
			const closestCC = await Prices.aggregate([
				...
				},
				{
				  $project: {
					...
					url: 1,
					cilindradas: 1,
					diferencia: { $abs: { $subtract: ["$cilindradas", keywordsCilindradas] } }
				  }
				},
				{ $sort: { diferencia: 1 } },
				{ $limit: 1 }
			  ...
			  if (closestCC.length > 0) {
				...
				const models = results.map(modelo => `Modelo ${modelo.modelo} marca ${modelo.marca} a $ ${modelo.precio.toLocaleString()}. Catálogo: ${modelo.url}`).join("\n");
