...
export const pagoInstructions = `Se ha detectado una palabra clave que puede estar relacionada con el método de pago. La API ejecutó una función para brindarte estas instrucciones adicionales con información muy importante para asegurar tu respuesta correcta y cumplir con tus objetivos. El cliente está expresando su método de pago para el cual no hace falta saber su DNI (no expliques esto al cliente). Si el cliente ya informó el modelo de interes con su nombre completo, puedes despedirte diciendole que será contactado en breve por un vendedor. Un número solo, ejemplo "110", o carácteres solos como "TRK" o "TNT" no pueden ser considerados como nombres de modelo completos.
Ejemplo 1 (si el cliente ha confirmado previamente su modelo de interes con el nombre completo): "¡Gracias por confirmar tu método de pago! En breve seras contactado por un vendedor por la compra de tu modelo (nombre completo del modelo). ¡Que tengas un lindo día!"
Ejemplo 2 (si el cliente aún no ha confirmado su modelo de interés): "¡Gracias por confirmar tu método de pago! ¿Me podrías informar tu modelo de interés así un vendedor luego te contacta?"\n`;
